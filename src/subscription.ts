...
const OpenRouterKeySchema = z.object({
  data: z.object({
    ...
    rate_limit: z
      .object({
        // OpenRouter can return unexpected values here (e.g. -1, 0, string, null).
        // Parse loosely, then normalize in code.
        requests: z.union([z.number(), z.string(), z.null()]).optional(),
      })
      .partial()
      .nullable()
      .optional(),
...
function normalizeRateLimitPerMinute(
...
export async function detectTier(
  ...
  const detectedLimit = normalizeRateLimitPerMinute(
    parsed.data.rate_limit?.requests,
  );
  ...
  return {
    ...
    rateLimitPerMinute: detectedLimit ?? fallbackRateByTier(isFreeTier),
  };
}
